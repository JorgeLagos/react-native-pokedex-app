  ...
  return (
    <Pressable style={{ flex: 1 }} onPress={() => navigation.navigate('PokemonScreen', { pokemonId: pokemon.id })}>
      <Card style={[style.cardContainer, { backgroundColor: pokemon.color }]}>
        <Text style={style.name} variant="bodyLarge" lineBreakMode="middle">
          {pokemon.name}
          {'\n#' + pokemon.id}
        </Text>
        <View style={style.pokeballContainer}>
          <Image source={require('../../../assets/pokeball-light.png')} style={style.pokeball} />
        </View>
        <FadeInImage uri={pokemon.avatar} style={style.pokemonImage} />
        <Text style={[style.name, { marginTop: 35 }]}>{pokemon.types[0]}</Text>
      </Card>
    </Pressable>
  )
}
